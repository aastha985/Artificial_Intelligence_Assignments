...
main:-  
    ...
    input_career(Career),
    input_prereqs(PrereqsCompleted),
    input_cgpa(CGPA),
    input_BTP(BTP),
    suggest_courses(Career,Semester,PrereqsCompleted,CGPA,BTP).
...
input_prereqs(PrereqsCompleted):-
    nl,nl,write("Please enter the prerequisites you have completed-"),
    write(". Enter 'endl.' when done."),nl,
    input_list(PrereqsCompleted).

suggest_courses(Career,Semester,PrereqsCompleted,CGPA,BTP):-
    elective(Career,Semester,Courses),
    find_courses(Courses,PrereqsCompleted,CGPA,BTP).
...
recommend_current_course(R,H1,CGPA,BTP):-
    length(R,N),
    (
        N=:=0, (CGPA>=8;BTP;easyMedium(H1)) -> assertz(course(H1)),nl
        ;write("")
    ).
